...
class Worker < ActiveRecord::Base
  ...
  has_many :sites, through: :sites_workers
  ...
  def calculate_trust_score(date = nil)
    calls = self.activities.select { |a| a.call? }
    calls = calls.select { |c| c.created_at.to_date == date } if date
